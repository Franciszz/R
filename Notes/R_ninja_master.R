...
gpl = readLines(file.path(R.home(), "COPYING"))
gpl
anim <- readLines("tenyears.txt",encoding = "UTF-8")
anim[[1]]
library(wordcloud2)
...
anim2 <- unlist(anim)
#设置分词引擎
cutter <- worker(stop_word = "stop_words.utf8")
TL_content <- segment(code="tenyears.txt",cutter)
TL_content
#加载data.table
library(dplyr)
library(stringr)
...
#去掉出现3次以下的词语
myfreq2 <- subset(myfreq,myfreq$Freq >=30 & nchar(as.character(myfreq$ty))==2)
#词云制作
...
library(jiebaR)
library(jiebaRD)
cutter <- worker(stop_word = "data/stop_words.utf8")
# filter_segment(preW,stopwords)
segword <- segment("data/President Xi words.txt",cutter)
library(dplyr)
library(stringr)
preW <- readLines(con <- file(segword,encoding = "UTF-8")) %>%
  str_split(.,pattern=" ")
prewfreq <- table(preW)
prewfreq <- rev(sort(prewfreq)) %>% as.data.frame(.)
library(wordcloud2)
wordcloud2(prewfreq,color = brewer.pal(8,"Dark2"),
           fontFamily = "微软雅黑")
